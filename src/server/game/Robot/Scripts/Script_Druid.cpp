...
bool Script_Druid::Tank(Unit* pmTarget)
{
    ...
    case ShapeshiftForm::FORM_BEAR:
    {
        ...
        if (rage > 100)
        {
            ...
            if (pmTarget->IsNonMeleeSpellCast(true))
            {
                if (sourceAI->CastSpell(pmTarget, "Bash", MELEE_MAX_DISTANCE))
                {
                    return true;
...
bool Script_Druid::Healer(Unit* pmTarget)
{
    ...
    pmTarget->GetDispellableAuraList(me, DispelType::DISPEL_POISON, dispelList_poison);
    ...
    DispelChargesList dispelList_curse;
    pmTarget->GetDispellableAuraList(me, DispelType::DISPEL_CURSE, dispelList_curse);
    if (!dispelList_curse.empty())
    {
...
bool Script_Druid::HealMe()
{
    ...
    DispelChargesList dispelList_poison;
    me->GetDispellableAuraList(me, DispelType::DISPEL_POISON, dispelList_poison);
    if (!dispelList_poison.empty())
    {
    ...
    DispelChargesList dispelList_curse;
    me->GetDispellableAuraList(me, DispelType::DISPEL_CURSE, dispelList_curse);
    if (!dispelList_curse.empty())
    {
