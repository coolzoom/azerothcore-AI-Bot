...
bool MarketerConfig::GetValueHelper(const char* name, ACE_TString& result)
{
...
bool MarketerConfig::LoadInitial(char const* file)
{
    ASSERT(file);
...
bool MarketerConfig::LoadMore(char const* file)
{
    ASSERT(file);
    ASSERT(_config);
...
bool MarketerConfig::LoadData(char const* file)
{
...
std::string MarketerConfig::GetStringDefault(const char* name, const std::string& def, bool logUnused /*= true*/)
{
    ACE_TString val;
...
bool MarketerConfig::GetBoolDefault(const char* name, bool def, bool logUnused /*= true*/)
{
    ACE_TString val;
...
int MarketerConfig::GetIntDefault(const char* name, int def, bool logUnused /*= true*/)
{
    ACE_TString val;
...
float MarketerConfig::GetFloatDefault(const char* name, float def, bool logUnused /*= true*/)
{
    ACE_TString val;
...
std::list<std::string> MarketerConfig::GetKeysByString(std::string const& name)
{
    GuardType guard(_configLock);
...
bool MarketerConfig::StartMarketerSystem()
{
    ...
    if (!LoadInitial(MARKETER_CONFIG_FILE_NAME))
    {
